...
from src.pdf_styles import (
    get_font as _pdf_styles_get_font,
    make_header_footer as _pdf_styles_make_header_footer,
    graph_section as _pdf_styles_graph_section,
    scale_image as _pdf_styles_scale_image,
    safe_text as _safe,
)

logger = logging.getLogger(__name__)

# ── 日本語フォント設定 (src.pdf_styles に委譲) ──────────────────────────────────────────────────────

def _setup_japanese_font() -> tuple[str, str, str]:
    """pdf_styles.get_font() に委譲。後方互換のため署名を維持。"""
    try:
        regular = _pdf_styles_get_font(False)
        bold    = _pdf_styles_get_font(True)
        return regular, bold, "(via pdf_styles)"
    except Exception:
        return "", "", ""

    for font_name, font_path in []:  # unreachable – kept for type-checker
        p = Path(font_path)
        if not p.exists():
            continue
        try:
...
_draw_header_footer = _pdf_styles_make_header_footer("解析レポート")
...
# フレームフェーズ名マップ (key_frame_sheet_generator と同一)
_FRAME_PHASE_MAP: dict[str, str] = {
    "start":       "開始フレーム",
    "first":       "開始フレーム",
    "25pct":       "25% — 助走期",
    "50pct":       "50% — クロスステップ",
...
def _frame_phase_label(filename: str) -> str:
    name_lower = filename.lower()
    for key, label in _FRAME_PHASE_MAP.items():
...
def _build_frames_pages(report_dir: Path, styles: dict,
    ...
    """代表フレームを1ページ2枚・大きく表示するセクション。"""
    story = []
    story.append(Paragraph(_safe("代表フレーム"), styles["section_heading"]))
    story.extend(_hr(styles))
    ...
    if not frame_imgs:
        story.append(Paragraph(_safe("代表フレームが見つかりませんでした。"), styles["body"]))
        story.append(PageBreak())
        return story
    ...
    for i, img_path in enumerate(frame_imgs):
        ...
        label = _frame_phase_label(img_path.name)
        card: List = []
        card.append(Paragraph(_safe(label), styles["label"]))
        card.append(HRFlowable(width=CONTENT_W, color=MID_GRAY, thickness=0.3))
        card.append(Spacer(1, 0.1 * cm))
        w, h = _pdf_styles_scale_image(img_path, max_img_w, max_img_h)
        try:
            card.append(Image(str(img_path), width=w, height=h, hAlign="CENTER"))
            card.append(Paragraph(_safe(img_path.name), styles["caption"]))
        except Exception:
            card.append(Paragraph(_safe(f"⚠ 画像読み込みエラー: {img_path.name}"),
                                  styles["body_sm"]))
        try:
...
def _build_graphs_pages(report_dir: Path, styles: dict,
                        images_per_page: int = 1) -> List:
    """グラフを1ページ1枚・解説文付きで表示するセクション。"""
    story = []
    story.append(Paragraph(_safe("解析グラフ"), styles["section_heading"]))
    story.extend(_hr(styles))
    ...
    if not graph_imgs:
        story.append(Paragraph(_safe("グラフ画像が見つかりませんでした。"), styles["body"]))
        story.append(PageBreak())
        return story

    # 共有スタイルセットを作成（graph_section が要求するキーを含む）
    from src.pdf_styles import get_styles as _get_shared_styles
    shared = _get_shared_styles("RPT_")

    for i, img_path in enumerate(graph_imgs):
        if i > 0:
            story.append(PageBreak())
        block = _pdf_styles_graph_section(img_path, shared,
                                          max_img_w=CONTENT_W,
                                          max_img_h=10.5 * cm)
        try:
...
def generate_pdf_report_for_job(job_dir: Path) -> Path:
    ...
    story.extend(_build_frames_pages(report_dir, styles, images_per_page=2))
    story.extend(_build_graphs_pages(report_dir, styles, images_per_page=1))
