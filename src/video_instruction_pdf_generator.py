...
def _setup_japanese_font() -> tuple[str, str]:
    """Windows フォントから日本語フォントを探して ReportLab に登録する。

    Returns
    -------
    tuple[str, str]
        (標準フォント名, 太字フォント名)。登録できなかった場合は ("", "")。
    """
    for font_name, font_path in _WINDOWS_FONT_CANDIDATES:
        ...
        try:
            ...
            try:
                ...
                bold_name = font_name
            logger.info("[video_instruction_pdf] 日本語フォント登録完了: %s", font_path)
            return font_name, bold_name
        except Exception as exc:
            logger.warning("[video_instruction_pdf] フォント登録失敗 (%s): %s", font_path, exc)
    logger.info("[video_instruction_pdf] 日本語フォントなし。Helvetica にフォールバック。")
    return "", ""
...
def _safe(text: str) -> str:
    """日本語フォントが未登録の場合、Latin-1 非対応文字を '?' に置換する。"""
    if _JP_FONT:
        return text
    result: list[str] = []
    for ch in text:
...
def _draw_header_footer(canvas, doc):
    ...
    canvas.drawRightString(PAGE_W - MARGIN, PAGE_H - 0.72 * cm, "解析動画 説明書")
    # フッター帯
    canvas.setFillColor(MID_GRAY)
    canvas.rect(0, 0, PAGE_W, 0.9 * cm, fill=1, stroke=0)
    canvas.setFillColor(TEXT_GRAY)
    canvas.setFont("Helvetica", 7)
    canvas.drawString(MARGIN, 0.32 * cm, "Generated by Javelin Video Analysis  |  Not for medical use")
...
def generate_video_instruction_pdf_for_job(job_dir: Path) -> Path:
    ...
    doc = SimpleDocTemplate(
        ...
        leftMargin=MARGIN,
        rightMargin=MARGIN,
        topMargin=MARGIN + 1.1 * cm,   # ヘッダー帯の高さ分を追加
        bottomMargin=MARGIN + 0.9 * cm,  # フッター帯の高さ分を追加
        title="解析動画 説明書 — Javelin Video Analysis",
