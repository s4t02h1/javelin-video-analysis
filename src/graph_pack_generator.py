...
_GRAPH_DESCRIPTIONS: dict[str, tuple[str, str]] = {
    "right_wrist_height": (
        ...
        "縦軸は正規化高さ（0 = 画面下端 / 1 = 画面上端）で、横軸は時間（秒）です。"
        "グラフが最高点に達する時刻がリリースの候補になります。"
        "リリース直前まで手首が継続的に上昇しているか、"
        "途中で大きく沈み込んでいないかを確認しましょう。"
        "高さの可動域（最高点と最低点の差）が大きいほど、"
        "縦方向の体の使い方ができている参考指標になります。",
    ),
    "right_arm_trajectory": (
        "右腕の軌跡 / Right Arm Trajectory",
        "右肩・右肘・右手首の位置変化を2D平面上に重ねた図です。"
        "腕がどのような通り道を通っているか、"
        "肘の引きつけや手首のスナップの動きを視覚的に確認できます。"
        "スムーズな曲線を描いているか、動作に乱れや引っかかりがないかが"
        "チェックのポイントです。"
        "撮影角度や姿勢推定精度の影響を受けるため、参考可視化として活用してください。",
    ),
    "torso_center_trajectory": (
        "体幹中心の移動 / Torso Center Trajectory",
        "肩の中心と腰の中心の移動軌跡を示しています。"
        "助走からリリースに向けて体幹がどの方向へ移動しているか、"
        "肩と腰の動きにずれがないかを確認するための参考資料です。"
        "前方への体重移動が適切に行われているか、"
        "体が横ぶれしていないかのチェックに使用できます。"
        "カメラの向きや画角の影響を受けるため、振り返り用として扱ってください。",
    ),
    "right_wrist_speed": (
        "右手首の速度変化 / Right Wrist Speed",
        "各フレームにおける右手首の推定速度変化を示しています。"
        "速度が最大になる時刻がリリース動作の候補です。"
        "助走から加速→リリースのピークに向けて速度が上がっているか、"
        "ピーク後に急激に速度が落ちているかを確認できます。"
        "数値は2D動画からの参考推定値であり、実際の球速とは異なります。",
    ),
    "arm_chain_speed": (
        "運動連鎖の速度変化 / Arm Chain Speed",
        "右肩・右肘・右手首の速度を同一グラフに重ねて表示しています。"
        "肩 → 肘 → 手首の順に速度のピークが来ているか（運動連鎖）を確認できます。"
        "末端の手首に向かうにつれて最大速度が大きくなっていれば、"
        "体全体のエネルギーが効率よく伝達できているサインです。"
        "ピークのタイミングが重なっている場合は、タイミングの調整が課題になることがあります。",
    ),
}
